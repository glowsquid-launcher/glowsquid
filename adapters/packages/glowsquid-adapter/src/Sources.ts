...
async function updateAdapter(adapterID: string, path: string) {
  const adapter = checkIfAdapterExists(adapterID)
  ...
  const asset = latestRelease.data.assets.find(
    (asset) => asset.label === `${adapter.config.id}.js`,
  )
  if (!asset) throw new Error(`asset named ${adapter.config.id}.js not found.`)
...
export async function removeAdapter(pathToSource: string) {
  const newSourcesList = sourcesList.filter((source) => source !== pathToSource)
...
export function getAdapter(id: string) {
  return checkIfAdapterExists(id)
}
